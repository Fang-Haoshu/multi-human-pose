...
function M.parse(arg)
    ...
    cmd:option('-addDPG',           false, 'use DPG to train SPPE, Options: true | false')
    cmd:option('-addParallelSPPE',  false, 'add parallel SPPE during training, Options: true | false')
    cmd:option('-addSTN',           false, 'add SSTN to a trained model, Options: true | false')
    cmd:option('-loadModel',      'umich-stacked-hourglass.t7','Provide full path to a previously trained model')
    ...
    cmd:option('-snapshot',            4, 'How often to take a snapshot of the model (0 = never)')
    ...
    cmd:text()
    cmd:option('-nEpochs',           16, 'Total number of epochs to run')
    cmd:option('-trainIters',       4000, 'Number of train iterations per epoch')
    cmd:option('-trainBatch',          4, 'Mini-batch size')
    cmd:option('-validIters',       1500, 'Number of validation iterations per epoch')
