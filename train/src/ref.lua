...
-- Project directory
projectDir = paths.concat(os.getenv('HOME'),'/git/multi-human-pose/train')
...
for _,l in ipairs(annotLabels) do
    ...
    if opt.dataset == 'mpii' and l == 'valid' and opt.finalPredictions == 1 then
        ...
        a = hdf5.open(opt.dataDir .. '/annot/' .. 'train-full' .. '.h5')
        namesFile = io.open(opt.dataDir .. '/annot/' .. 'train-full' .. '_images.txt')
    end
    annot[l] = {}
