...
function step(tag)
    ...
    for i=1,r.iters do
        ...
        -- Validation: Get flipped output
            ...
            flippedOut = applyFn(function (x) return flip_(shuffleLR_(x)) end, {flippedOut[1],flippedOut[2]})
            output = applyFn(function (x,y) return x:add(y):div(2) end, {output[1],output[2]}, {flippedOut[1],flippedOut[2]})
        ...
        -- Calculate accuracy
        local acc = accuracy({output[1],output[2]}, label)
        avgLoss = avgLoss + err
        avgAcc = avgAcc + acc
    end
