...
function GetAlpha:updateOutput(input)
   ...
   self.determinants = torch.CudaTensor(theta:size(1),1):zero()
   ...
   for i=1,batchsize do
      ...
      self.det[i] = theta_first_two_colomn[{1,1}]*theta_first_two_colomn[{2,2}] - theta_first_two_colomn[{1,2}]*theta_first_two_colomn[{2,1}]
...
function GetAlpha:updateGradInput(_input,gradOutput)
   ...
   assert(gradOutput:size() == _input:size())
   local batchsize = input:size(1)
   local alpha = self.output
   for i = 1,batchsize do
     ...
     local dalpha1_2 = (-1)*torch.mv(dy[{{},3}], theta[{{},3}]:t())
     dalpha1_2 += dy[{{},{1,2}] 
     local dtheta1_2 = torch.CudaTensor(2,3)
     dtheta1_2[{1,1}] = theta[{2,2}]*dalpha1_2[{2,2}]
     dtheta1_2[{1,2}] = theta[{2,1}]*dalpha1_2[{1,2}]
     dtheta1_2[{2,1}] = theta[{1,2}]*dalpha1_2[{2,1}]
     dtheta1_2[{2,2}] = theta[{1,1}]*dalpha1_2[{1,1}]
     dtheta1_2:cmul(1/(self.det[i]*self.det[i]))
