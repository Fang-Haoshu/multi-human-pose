...
if opt.GPU == -1 then
	...
	------------------------------------------------------------
	-------- Here we load Siamese network model ----------------
	------------------------------------------------------------

	Siamese_model = torch.load('siamese-model/final_model.t7'):cuda()
...
if not optimState then
    ...
    optimState.learningRates = torch.Tensor(6):fill(0)
    optimState.learningRates[2] =  opt.LR; optimState.learningRates[6] =opt.LR;
    optimState.learningRates[4] = opt.LR; optimState.learningRates[5] = opt.LR;
end
...
for _,l in ipairs(annotLabels) do
    ...
    if l == 'valid' then
        ...
        a = hdf5.open(opt.dataDir .. '/annot/train.h5')
        namesFile = io.open(opt.dataDir .. '/annot/train_images.txt')
    end
    annot[l] = {}

    -- Read in annotation information
    local tags = {'part',  'bndbox'}
